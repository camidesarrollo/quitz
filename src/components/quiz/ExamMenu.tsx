...
import { PASS_THRESHOLD, getWrongQuestionsFromHistory } from "@/lib/quiz-utils";
import {
  getSRSessionQuestions,
  getSRStats,
  SR_NEW_PER_SESSION,
  SR_REVIEW_PER_SESSION,
...
export function ExamMenu() {
  ...
  const { history, markedQuestionIds, startSession, startSessionWithQuestions, clearSession } =
    useQuizStore();
  const { cards: srCards } = useSRStore();
  const wrongQuestions = getWrongQuestionsFromHistory(history);
  const allQuestions = getAllQuestions();
  const markedQuestions = allQuestions.filter((q) => markedQuestionIds.includes(q.id));
  ...
  function handleStart() {
    ...
    const modePrefix =
      mode === "sniper" ? "Sniper · " : mode === "mental-state" ? "Estado Mental · " : "";
  ...
  function handleStartSR() {
    ...
    clearSession();
    const parts: string[] = [];
    if (srSession.reviewCount > 0)
      parts.push(`${srSession.reviewCount} repaso${srSession.reviewCount !== 1 ? "s" : ""}`);
    if (srSession.newCount > 0)
      parts.push(`${srSession.newCount} nueva${srSession.newCount !== 1 ? "s" : ""}`);
    const config: QuizConfig = {
      courseId: COURSE.id,
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                {poolSize} preguntas · Umbral {PASS_THRESHOLD}%
              </p>
            </div>
          </div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">Examen</h1>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-colors border-2",
                    mode === value
                      ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300"
                      : "border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600"
                  )}
            ...
            <div className="flex items-center gap-2 py-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 px-4">
              ...
              {srStats.nextReviewAt && (
                <p className="text-xs text-slate-400 dark:text-slate-500 ml-auto">
                  Próximo repaso{" "}
                  {new Date(srStats.nextReviewAt).toLocaleDateString("es-AR", {
                    weekday: "short",
                    day: "numeric",
                    month: "short",
                  })}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-red-200 dark:border-red-900/60 p-5 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  {wrongQuestions.length} pregunta
                  {wrongQuestions.length !== 1 ? "s" : ""} incorrecta
                  {wrongQuestions.length !== 1 ? "s" : ""} únicas de todo tu historial
                </p>
              </div>
            </div>
            <button
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-amber-200 dark:border-amber-800/60 p-5 mb-4">
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-xl bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center shrink-0">
                <Bookmark
                  size={18}
                  className="text-amber-500 dark:text-amber-400 fill-amber-200 dark:fill-amber-900/60"
                />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-50">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  {markedQuestions.length} pregunta
                  {markedQuestions.length !== 1 ? "s" : ""} guardada
                  {markedQuestions.length !== 1 ? "s" : ""} para repasar
                </p>
              </div>
            </div>
            <button
